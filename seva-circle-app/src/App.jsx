...
export default function KaryakarHub() {
  const [stage, setStage] = useState("login"); // login | team-setup | onboarding | dashboard
  const [passcode, setPasscode] = useState("");
  const [loginError, setLoginError] = useState(false);
  const [teamPasscodes, setTeamPasscodes] = useState({ kishori: "KKT1234", balika: "BKT1234" });
  const [pendingTeam, setPendingTeam] = useState(null);
  const [pendingMissingRoles, setPendingMissingRoles] = useState([]);

  const [teams, setTeams] = useState(DEFAULT_TEAMS);
  const [rolesByTeam, setRolesByTeam] = useState(DEFAULT_ROLES);
  ...
  const [ekadashiDates, setEkadashiDates] = useState({});
  ...
  const rootProps = {
    ...
    feedbackNotes, setFeedbackNotes, leaderTasks, setLeaderTasks,
...
function CalendarTab(props) {
  ...
  function deleteTask(id) {
    ...
    showUndoToast?.("Task deleted", () => {
      if (removed) setLeaderTasks((prev) => [...prev, removed]);
    });
  }
...
/* ---------------------------------------------------------------
   ROSTER TAB
--------------------------------------------------------------- */
function RosterTab({ members, currentUser, setCurrentUser, setMembers, teams, showToast, feedbackNotes, setFeedbackNotes, attendance }) {
...
function SwapTab({ swaps, setSwaps, currentUser, members, showToast, showUndoToast }) {
  ...
  function cancelSwap(id) {
    ...
    showUndoToast?.("Swap request canceled", () => {
      if (removed) setSwaps((prev) => [removed, ...prev]);
    });
  }
...
function IdeasTab({ ideas, setIdeas, currentUser, members, showToast, showUndoToast }) {
  ...
  function toggleExecute(id) {
    setIdeas((prev) => prev.map((i) => {
      ...
      return { ...i, executors, executing: executors.length > 0 };
    }));
  }
  function deleteIdea(id) {
    ...
    showUndoToast?.("Idea deleted", () => {
      if (removed) setIdeas((prev) => [removed, ...prev]);
    });
  }
...
function ProfileModal({ currentUser, setCurrentUser, setMembers, members, setProfileOpen, onLogout, teamPasscodes, setTeamPasscodes, teams, setTeams, rolesByTeam, setRolesByTeam, theme, setTheme, showToast, showUndoToast, feedbackNotes, setFeedbackNotes }) {
  ...
  function deleteNote(id) {
    ...
    showUndoToast?.("Note deleted", () => {
      if (removed) setFeedbackNotes((prev) => [...prev, removed]);
    });
  }
  ...
  function saveTeamInfo() {
    ...
    setTeams((prev) => prev.map((t) => (t.id === currentUser.team ? { ...t, name: teamNameEdit.trim(), description: teamBlurbEdit, motto: teamMottoEdit } : t)));
    setTeamSaved(true);
  ...
  function addTeam() {
    ...
    setTeams((prev) => [...prev, { id, name: newTeamName.trim(), color: newTeamColor, description: "", motto: "" }]);
    setRolesByTeam((prev) => ({ ...prev, [id]: ["Main", "Co", "Karyakar", "Admin", "PC"] }));
    setMembers((prev) => {
      const c1 = SWATCHES.find((c) => !isColorTaken(c, prev)) || SWATCHES[0];
      const withMain = [...prev, { id: "m-" + Date.now(), name: foundingMainName.trim(), team: id, role: "Main", color: c1, seva: "" }];
      const c2 = SWATCHES.find((c) => !isColorTaken(c, withMain)) || SWATCHES[1];
      return [...withMain, { id: "m-" + (Date.now() + 1), name: foundingCoName.trim(), team: id, role: "Co", color: c2, seva: "" }];
    });
  ...
  function promoteMember() {
    ...
    setMembers((prev) => prev.map((m) => (m.id === promoteMemberId ? { ...m, role: promoteRole } : m)));
  ...
  function addRole() {
    ...
    setRolesByTeam((prev) => {
      ...
      return { ...prev, [roleTeamId]: [...existing, label] };
    });
